...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (status === 'loading') {
    return <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-[#1f2937] dark:bg-[#030712]">Chargement...</div>;
  }
  ...
  const getPageTitle = () => {
    if (pathname === '/admin/dashboard') return t('navDashboard', { fallback: 'Dashboard' });
    if (pathname === '/admin/users') return t('navUsers', { fallback: 'User Management' });
    if (pathname === '/admin/formations') return t('navCourses', { fallback: 'Course Management' });
    if (pathname === '/admin/settings') return t('navSettings', { fallback: 'Settings' });
    return t('adminSpace', { fallback: 'Admin Space' });
  };
        ...
        <div className="h-[72px] px-6 flex items-center border-b border-slate-100 dark:border-gray-800 dark:border-gray-800">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="text-xl font-extrabold text-slate-900 dark:text-white dark:text-white tracking-tight">
              HR-<span className="text-[#0066FF] dark:text-blue-400">Trainer</span> <span className="text-sm font-medium text-slate-400">Admin</span>
            </span>
          </Link>
        </div>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <header className="h-[72px] bg-white dark:bg-[#111827] dark:bg-[#030712] border-b border-slate-200 dark:border-gray-700 dark:border-gray-800 flex items-center justify-between px-6 md:px-8 flex-shrink-0 z-10 transition-colors">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-bold text-slate-900 dark:text-white dark:text-white">{getPageTitle()}</span>
          </div>
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            <form onSubmit={handleSearch} className="hidden md:flex relative">
              ...
              <input 
                ...
                placeholder="Search..." 
                className="w-64 pl-9 pr-4 py-2 bg-slate-50 dark:bg-[#1f2937] dark:bg-[#111827] border border-slate-200 dark:border-gray-700 dark:border-gray-700 rounded-full text-sm text-slate-900 dark:text-white dark:text-white focus:outline-none focus:ring-2 focus:ring-[#0066FF]/20 focus:border-[#0066FF] transition placeholder:text-slate-400" 
              />
            </form>
            ...
            <Link href="/admin/settings" className="flex items-center gap-3 pl-4 sm:pl-6 border-l border-slate-200 dark:border-gray-700 hover:opacity-80 transition cursor-pointer">
              <div className="hidden sm:block text-right">
                <div className="text-sm font-bold text-slate-900 dark:text-white leading-tight">{adminProfile?.nom || session?.user?.name}</div>
