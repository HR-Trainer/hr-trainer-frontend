    ...
    <div className="max-w-6xl mx-auto pb-20">
      ...
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-slate-200 dark:border-gray-700">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-slate-500 dark:text-gray-400 font-medium">{data?.module?.formation?.titre || 'Formation'}</span>
          <ChevronRight size={14} className="text-slate-300" />
          <span className="font-bold text-slate-900 dark:text-white">{data?.module?.titre}</span>
        </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 space-y-8">
              ...
              {data?.module?.typeContenu === 'VIDEO' ? (
                data.module.contenuUrl ? (
                  ...
                  <div className="p-8 z-0">
                    <div className="w-16 h-16 rounded-full bg-white dark:bg-[#111827]/10 flex items-center justify-center text-white mx-auto mb-6 group-hover:bg-white dark:bg-[#111827]/20 transition">
          ...
          <div className="bg-white dark:bg-[#111827] rounded-[1.5rem] p-6 sm:p-8 border border-slate-100 dark:border-gray-800 shadow-sm">
            ...
            <div className="text-slate-600 dark:text-gray-300 leading-relaxed text-sm space-y-4 whitespace-pre-wrap">
              {data?.module?.description || "Le contenu de ce module s'affichera ici."}
            </div>
            {data?.module?.typeContenu === 'LECTURE' && data?.module?.contenuUrl && (
              <div className="mt-6 pt-6 border-t border-slate-100 dark:border-gray-800">
                <a href={data.module.contenuUrl} target="_blank" className="inline-flex items-center gap-2 px-6 py-3 bg-emerald-50 text-emerald-700 font-bold rounded-xl text-sm hover:bg-emerald-100 transition">
                  <FileText size={18} /> Télécharger / Voir le document
                </a>
              </div>
            )}
          </div>
          ...
          {data?.module?.quiz && (
            <div className="bg-white dark:bg-[#111827] rounded-[1.5rem] p-6 sm:p-8 border border-slate-100 dark:border-gray-800 shadow-sm" id="quiz">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-xl font-bold text-slate-900 dark:text-white">Quiz de validation</h2>
              </div>

              {quizSubmitted || data?.progression?.score != null ? (
                <div className="space-y-8">
                  <div className={`p-6 rounded-xl border flex items-center justify-between ${score !== null && score >= 70 || (data?.progression?.score && data?.progression?.score >= 70) ? 'bg-emerald-50 border-emerald-100' : 'bg-red-50 border-red-100'}`}>
                    <div>
                      <h3 className={`text-lg font-bold mb-1 ${score !== null && score >= 70 || (data?.progression?.score && data?.progression?.score >= 70) ? 'text-emerald-800' : 'text-red-800'}`}>
                      ...
                      <p className={`font-medium ${score !== null && score >= 70 || (data?.progression?.score && data?.progression?.score >= 70) ? 'text-emerald-600' : 'text-red-600'}`}>
                        Score : {score ?? data?.progression?.score} %
                      </p>
                    </div>
                    <div className={`text-xl font-black ${score !== null && score >= 70 || (data?.progression?.score && data?.progression?.score >= 70) ? 'text-emerald-600' : 'text-red-600'}`}>
                      {score !== null && score >= 70 || (data?.progression?.score && data?.progression?.score >= 70) ? '✓ Validé' : '✗ À revoir'}
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                    {data.module.quiz.questions.map((q: any, i: number) => {
                      const selectedOptionId = quizAnswers[q.id];
                      return (
                        <div key={q.id} className="p-6 rounded-2xl border border-slate-100 dark:border-gray-800 bg-white dark:bg-[#111827] shadow-sm space-y-4">
                          <h4 className="font-bold text-slate-900 dark:text-white text-base">
                            <span className="text-[#0066FF] mr-1">Q{i + 1}.</span> {q.texte}
                          </h4>
                          <div className="space-y-3">
                            {q.options.map((opt: any) => {
                              const isSelected = selectedOptionId === opt.id;
                              const isCorrect = opt.estCorrecte;
                              
                              let optionClass = "border-slate-200 dark:border-gray-700 text-slate-500 dark:text-gray-400 opacity-70";
                              let icon = null;
                              ...
                              if (isCorrect) {
                                ...
                                icon = <span className="text-red-600 font-bold text-lg leading-none">×</span>;
                              }

                              return (
                                <div key={opt.id} className={`flex items-center justify-between p-4 rounded-xl border ${optionClass}`}>
                                  <div className="flex items-center gap-3">
                                    <div className={`w-4 h-4 rounded-full border flex-shrink-0 ${isCorrect ? 'border-emerald-500 bg-emerald-500' : isSelected ? 'border-red-500 bg-red-500' : 'border-slate-300'}`}>
                                      {(isCorrect || isSelected) && <div className="w-full h-full rounded-full border-2 border-white"></div>}
                                    </div>
                          ...
                          {q.explication && (
                            <div className="mt-4 p-4 rounded-xl bg-amber-50 text-amber-800 text-sm flex items-start gap-3 border border-amber-100">
                  ...
                  <button 
                    ...
                    className="w-full py-4 border-2 border-slate-200 dark:border-gray-700 text-slate-700 dark:text-gray-200 hover:border-[#0066FF] hover:text-[#0066FF] font-bold rounded-xl transition bg-white dark:bg-[#111827]"
                  >
                    Réessayer le quiz
                  </button>
                </div>
                ...
                <form onSubmit={handleQuizSubmit} className="space-y-8">
                  {data.module.quiz.questions.map((q: any, i: number) => (
                    <div key={q.id}>
                      <h4 className="font-bold text-slate-900 dark:text-white mb-4">{i + 1}. {q.texte}</h4>
                      <div className="space-y-3">
                        {q.options.map((opt: any) => (
                          <label key={opt.id} className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition ${quizAnswers[q.id] === opt.id ? 'border-[#0066FF] bg-blue-50' : 'border-slate-200 dark:border-gray-700 hover:bg-slate-50 dark:bg-[#1f2937]'}`}>
                            <input 
                              type="radio" 
                              name={q.id} 
                              value={opt.id} 
                              ...
                              required
                            />
                            <span className={`text-sm font-medium ${quizAnswers[q.id] === opt.id ? 'text-[#0066FF]' : 'text-slate-700 dark:text-gray-200'}`}>{opt.texte}</span>
                          </label>
                        ))}
                      </div>
                    </div>
          ...
          <div className="flex items-center justify-between pt-6 border-t border-slate-100 dark:border-gray-800">
            {prevModule ? (
              ...
              <span className="flex items-center gap-2 px-6 py-3 border border-slate-100 dark:border-gray-800 text-sm font-bold text-slate-300 rounded-xl">
                <ChevronLeft size={16} /> Précédent
              </span>
            )}
            
            <Link href="/mon-espace" className="text-sm font-medium text-slate-400 hover:text-slate-600 dark:text-gray-300 transition flex items-center gap-2">
            ...
            <div className="overflow-y-auto custom-scrollbar flex-1 p-2">
              {moduleList.map((mod: any, index: number) => {
                ...
                if (mod.typeContenu === 'LECTURE') {
                  TypeIcon = FileText;
                  typeLabel = "Lecture";
                  typeColor = "text-emerald-500 dark:text-emerald-400";
                  typeBg = "bg-emerald-50 dark:bg-emerald-500/10";
                } else if (mod.typeContenu === 'QUIZ' || mod.quiz) {
                  typeLabel = "Quiz";
                  typeColor = "text-purple-500 dark:text-purple-400";
                  typeBg = "bg-purple-50 dark:bg-purple-500/10";
                } else {
                  typeColor = "text-[#0066FF] dark:text-blue-400";
                  typeBg = "bg-blue-50 dark:bg-blue-500/10";
                }

                return (
                  <Link key={mod.id} href={`/formations/${formationId}/modules/${mod.id}`} className={`flex gap-4 p-4 rounded-xl transition ${isActive ? 'bg-slate-50 dark:bg-slate-800' : 'hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                    <div className="flex-shrink-0 mt-0.5">
                      {isActive ? (
                        ...
                        <div className="w-6 h-6 rounded-full border-2 border-slate-200 dark:border-gray-700 flex items-center justify-center text-[10px] font-bold text-slate-400">
                          {index + 1}
                        </div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className={`text-sm font-bold truncate ${isActive ? 'text-[#0066FF]' : 'text-slate-700 dark:text-gray-200'}`}>
                        {mod.titre}
                      </h4>
