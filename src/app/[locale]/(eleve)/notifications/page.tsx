...
export default function Notifications() {
  ...
  useEffect(() => {
    if (session?.user?.email) {
      const savedPrefs = localStorage.getItem(`eleve_prefs_${session.user.email}`);
      if (savedPrefs) {
  ...
  const togglePref = (key: keyof typeof prefs) => {
    ...
    setPrefs(newPrefs);
    if (session?.user?.email) {
      localStorage.setItem(`eleve_prefs_${session.user.email}`, JSON.stringify(newPrefs));
    }
  };
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div>
        ...
        <p className="text-slate-500 dark:text-gray-400 font-medium text-[15px]">Choose how you want to be contacted</p>
      </div>
      ...
      <div className="bg-white dark:bg-[#111827] rounded-[1.5rem] shadow-sm border border-slate-100 dark:border-gray-800 p-8">
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between gap-4">
            <div>
              <h3 className="font-bold text-slate-900 dark:text-white text-sm">Course Reminders</h3>
              <p className="text-xs text-slate-500 dark:text-gray-400 mt-1 font-medium">When you haven't made progress for 3 days</p>
            </div>
            <button onClick={() => togglePref('rappels')} className={`w-12 h-6 rounded-full transition-colors relative flex-shrink-0 ${prefs.rappels ? 'bg-[#0066FF]' : 'bg-slate-200'}`}>
              <span className={`absolute top-1 w-4 h-4 rounded-full bg-white dark:bg-[#111827] transition-all ${prefs.rappels ? 'left-7' : 'left-1'}`}></span>
            </button>
          </div>

          <div className="flex items-center justify-between gap-4">
            <div>
              <h3 className="font-bold text-slate-900 dark:text-white text-sm">New Courses</h3>
              <p className="text-xs text-slate-500 dark:text-gray-400 mt-1 font-medium">Alerts when a new course is available</p>
            </div>
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <h3 className="font-bold text-slate-900 dark:text-white text-sm">Email Notifications</h3>
              <p className="text-xs text-slate-500 dark:text-gray-400 mt-1 font-medium">Weekly summary of your progress</p>
            </div>
