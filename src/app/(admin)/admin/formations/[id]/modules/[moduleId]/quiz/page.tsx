  ...
  return (
    <div className="space-y-6 max-w-4xl relative">
      <div className="flex items-center gap-4 mb-6">
        <Link href={`/admin/formations/${courseId}`} className="p-2 bg-white rounded-full border border-slate-200 text-slate-500 hover:text-slate-900 hover:shadow-sm transition">
          <ChevronLeft size={20} />
        </Link>
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900">Quiz Builder</h1>
          <p className="text-sm font-medium text-slate-500">Add questions and options for this module</p>
        </div>
      </div>

      <div className="space-y-8">
        {questions.map((q, qIndex) => (
          <div key={qIndex} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 relative">
            <div className="flex justify-between items-start mb-4">
              <h2 className="text-lg font-bold text-slate-800">Question {qIndex + 1}</h2>
              {questions.length > 1 && (
            ...
            <input
              ...
              className="w-full px-4 py-3 mb-6 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#0066FF]/20 focus:border-[#0066FF] transition font-medium text-slate-800"
            />
              ...
              {q.options.map((opt: any, oIndex: number) => (
                <div key={oIndex} className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    className={`flex-1 px-4 py-2 border rounded-lg text-sm focus:outline-none transition ${opt.estCorrecte ? 'border-green-300 bg-green-50/30' : 'border-slate-200'}`}
                  />
                  {q.options.length > 2 && (
      ...
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl border border-slate-200 shadow-sm sticky bottom-6">
        <button onClick={handleAddQuestion} className="flex items-center gap-2 px-5 py-2.5 bg-slate-100 text-slate-700 font-bold rounded-xl hover:bg-slate-200 transition">
