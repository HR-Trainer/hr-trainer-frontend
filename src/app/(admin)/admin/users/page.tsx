    ...
    <div className="space-y-6 max-w-6xl relative">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 font-medium">Manage students, enterprise accounts, and admins.</p>
        </div>
        <button 
          onClick={() => setShowAddModal(true)}
          className="bg-[#0066FF] text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-md shadow-[#0066FF]/20 hover:bg-blue-700 transition"
        >
          + Add New User
        </button>
      </div>
                ...
                <tr key={user.id} className="hover:bg-slate-50 transition">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="font-semibold text-slate-700 text-xs">{user.profil === 'ENTREPRISE' ? 'Company' : 'Individual'}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <button 
                      onClick={() => toggleUserPremium(user.id, user.statutAcces)}
                      className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider transition ${user.statutAcces === 'PAYANT' ? 'bg-amber-100 text-amber-700 hover:bg-amber-200' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                    >
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleAddUser} className="p-6 space-y-4">
              <div>
                ...
                <input required type="text" value={newUser.nom} onChange={e => setNewUser({...newUser, nom: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#0066FF]/20 focus:border-[#0066FF]" placeholder="Jane Doe" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1.5">Email Address</label>
                <input required type="email" value={newUser.email} onChange={e => setNewUser({...newUser, email: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#0066FF]/20 focus:border-[#0066FF]" placeholder="jane@example.com" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1.5">Password</label>
                <input required type="password" value={newUser.password} onChange={e => setNewUser({...newUser, password: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#0066FF]/20 focus:border-[#0066FF]" placeholder="••••••••" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1.5">System Role</label>
                  <select value={newUser.role} onChange={e => setNewUser({...newUser, role: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#0066FF]/20 focus:border-[#0066FF]">
                ...
                {newUser.role === 'ELEVE' && (
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">Profile Type</label>
                    <select value={newUser.profil} onChange={e => setNewUser({...newUser, profil: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#0066FF]/20 focus:border-[#0066FF]">
              ...
              <div className="pt-4 flex justify-end gap-3">
                <button type="button" onClick={() => setShowAddModal(false)} className="px-4 py-2 text-sm font-bold text-slate-500 hover:text-slate-700 transition">Cancel</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-8 text-center relative">
              ...
              <h2 className="text-xl font-extrabold text-slate-900 mb-2">Delete User?</h2>
              <p className="text-sm text-slate-500 mb-6">
                Are you sure you want to permanently delete <strong className="text-slate-800">{userToDelete?.nom}</strong>? This action cannot be undone and will erase all their progress.
              </p>
