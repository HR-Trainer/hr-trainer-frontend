  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div>
        ...
        <p className="text-slate-500 font-medium text-[15px]">Download certificates for your completed courses</p>
      </div>
        ...
        {data?.attestations?.length > 0 ? (
          <div className="space-y-4">
            {data.attestations.map((att: any) => (
              <div key={att.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-2xl border border-slate-100 hover:border-blue-200 hover:shadow-md hover:bg-blue-50/30 transition group">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-900 text-[15px]">{att.formation.titre}</h3>
                ...
                <button 
                  ...
                  className="flex items-center justify-center gap-2 px-5 py-2.5 bg-white border border-slate-200 text-[#0066FF] rounded-xl font-bold text-sm hover:bg-blue-50 hover:border-blue-200 transition disabled:opacity-50"
                >
          ...
          <div className="text-center py-12">
            ...
            <p className="text-slate-500 text-sm">Complete a course to 100% to unlock your first certificate.</p>
          </div>
        )}
      </div>
      ...
      <div className="overflow-hidden h-0 w-0 absolute opacity-0 pointer-events-none">
        {certData && (
          <div ref={certRef} className="w-[1123px] h-[794px] bg-white p-[40px] box-border relative font-sans">
            <div className="w-full h-full border-[10px] border-[#0066FF] p-[40px] box-border relative flex flex-col items-center justify-center text-center">
              ...
              <div className="absolute top-[60px] flex flex-col items-center">
                ...
                <h2 className="text-2xl font-black tracking-widest text-slate-800 uppercase">HR-Trainer Academy</h2>
              </div>

              {/* Title */}
              <h1 className="text-[60px] font-black text-[#0066FF] mt-12 mb-8 uppercase tracking-widest">Certificate of Completion</h1>
              
              <p className="text-2xl text-slate-500 font-medium mb-6">successfully awarded to</p>
              
              {/* Name */}
              <h2 className="text-[48px] font-extrabold text-slate-900 mb-12 border-b-2 border-slate-200 pb-4 px-12 inline-block">
                {session?.user?.name || 'Learner Name'}
              </h2>
              
              <p className="text-2xl text-slate-500 font-medium mb-6">for attending and completing the course:</p>
              ...
              <div className="flex justify-between items-end w-full px-12 absolute bottom-[80px]">
                <div className="text-left">
                  ...
                  <p className="text-xl text-slate-800 font-bold border-t-2 border-slate-200 pt-2 w-48">
                ...
                <div className="text-right">
                  ...
                  <div className="border-t-2 border-slate-200 pt-2 w-48 font-signature text-3xl text-slate-800">
                    M. HR-Trainer
                  </div>
                </div>
              </div>
