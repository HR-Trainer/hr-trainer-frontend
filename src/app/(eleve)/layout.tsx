...
export default function EleveLayout({ children }: { children: React.ReactNode }) {
  ...
  const navLinksEspace = [
    { href: '/mon-espace', label: 'Dashboard', icon: LayoutDashboard },
    { href: '/mes-formations', label: 'My Courses', icon: BookOpen, badge: stats.formations },
    { href: '/profil', label: 'My Profile', icon: User },
    { href: '/securite', label: 'Security', icon: Shield },
    { href: '/notifications', label: 'Notifications', icon: Bell, badge: stats.notifications },
  ];

  const navLinksGlobal = [
    { href: '/formations', label: 'Course Catalog', icon: GraduationCap },
    { href: '/entreprises', label: 'Enterprise', icon: Building },
    { href: '/contact', label: 'Contact', icon: Mail },
  ];

  const getPageTitle = () => {
    if (pathname === '/mon-espace') return 'Dashboard';
    if (pathname === '/mes-formations') return 'My Courses';
    if (pathname === '/profil') return 'My Profile';
    if (pathname === '/securite') return 'Security';
    if (pathname === '/notifications') return 'Notifications';
    if (pathname === '/attestations') return 'Certificates';
    if (pathname?.includes('/modules/')) return 'Current Module';
    return 'Learner Space';
  };
        ...
        <div className="h-[72px] px-6 flex items-center border-b border-slate-100">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="text-xl font-extrabold text-slate-900 tracking-tight">
              HR-<span className="text-[#0066FF]">Trainer</span>
            </span>
          </Link>
        </div>
                  ...
                  return (
                    <Link key={link.href} href={link.href} className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-bold transition ${isActive ? 'bg-[#0066FF] text-white shadow-md shadow-[#0066FF]/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}>
                      ...
                      {link.badge !== undefined && link.badge > 0 && (
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${isActive ? 'bg-white text-[#0066FF]' : 'bg-slate-100 text-slate-500'}`}>
                          {link.badge}
                        </span>
                      )}
                    </Link>
            ...
            <div className="mb-6">
              ...
              <nav className="space-y-1">
                {navLinksGlobal.map(link => (
                  <Link key={link.href} href={link.href} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold text-slate-500 hover:bg-slate-50 hover:text-slate-900 transition">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <header className="h-[72px] bg-white border-b border-slate-200 flex items-center justify-between px-6 md:px-8 flex-shrink-0 z-10">
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            ...
            <Link href="/profil" className="flex items-center gap-3 pl-4 sm:pl-6 border-l border-slate-200 hover:opacity-80 transition cursor-pointer">
              <div className="hidden sm:block text-right">
                <div className="text-sm font-bold text-slate-900 leading-tight">{userProfile?.nom || session?.user?.name}</div>
